...
# 数据集加载函数，指明数据集的位置并统一处理为imgheight*imgwidth的大小，同时设置batch
            #训练集目录   测试集目录       图片的高度     图片的宽度  设置图片每批加载的张数
...
def model_load(IMG_SHAPE=(224, 224, 3), class_num=12):
    ...
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.MaxPooling2D(2, 2),     #一层卷积一层池化，一层卷积一层池化，把它拉伸后送到全连接层。
        # 将二维的输出转化为一维
    ...
    # 输出模型信息
    model.summary()
    # 指明模型的训练参数，优化器随机梯度下降sgd优化器，损失函数为交叉熵损失函数
...
def train(epochs):
    ...
    model.save("models/cnn_vegebable2.h5")
    # 记录结束时间
    end_time = time()
